import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { addDemo, type Demo } from "../data/demos";

export default function DemoPage() {
    const navigate = useNavigate();
    const [form, setForm] = useState<Omit<Demo, "id">>({
        nome: "",
        codigoProjeto: "",
        horizontal: "",
        nomeComercial: "",
        vertical: "",
        urlFotoComercial: "",
        nomeImagemDocker: "",
    });

    const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { name, value } = e.target;
        setForm((p) => ({ ...p, [name]: value }));
    };

    const onSubmit = async (e: React.FormEvent) => {
        e.preventDefault();

        // Tudo obrigatório menos urlFotoComercial
        if (
            !form.nome.trim() ||
            !form.codigoProjeto.trim() ||
            !form.horizontal.trim() ||
            !form.nomeComercial.trim() ||
            !form.vertical.trim() ||
            !form.nomeImagemDocker.trim()
        ) {
            alert("Preenche os campos obrigatórios.");
            return;
        }

        const created = await addDemo(form);
        try {
            navigate(`/demos/${created.id}`, { replace: true });
        } catch {
            navigate("/demos", { replace: true });
        }
    };

    return (
        <div className="page-container">
            <button className="btn-back-outline" onClick={() => navigate("/demos")}>
                <i className="bi bi-arrow-left" />
                <span>Voltar</span>
            </button>

            <div className="card mt-16">
                <div className="card-header">
                    <h2 className="card-title">Adicionar Nova Demo</h2>
                </div>

                <div className="card-body">
                    <form className="form-grid" onSubmit={onSubmit}>
                        <div className="form-row">
                            <div className="form-group">
                                <label>Nome *</label>
                                <input
                                    name="nome"
                                    value={form.nome}
                                    onChange={onChange}
                                    placeholder="Ex.: Demo 360"
                                    required
                                />
                            </div>

                            <div className="form-group">
                                <label>Código do Projeto *</label>
                                <input
                                    name="codigoProjeto"
                                    value={form.codigoProjeto}
                                    onChange={onChange}
                                    placeholder="Ex.: LTP-001"
                                    required
                                />
                            </div>
                        </div>

                        <div className="form-row">
                            <div className="form-group">
                                <label>Horizontal *</label>
                                <input
                                    name="horizontal"
                                    value={form.horizontal}
                                    onChange={onChange}
                                    placeholder="Ex.: Marketing"
                                    required
                                />
                            </div>

                            <div className="form-group">
                                <label>Nome do Comercial *</label>
                                <input
                                    name="nomeComercial"
                                    value={form.nomeComercial}
                                    onChange={onChange}
                                    placeholder="Ex.: Ana Silva"
                                    required
                                />
                            </div>
                        </div>

                        <div className="form-row">
                            <div className="form-group">
                                <label>Vertical *</label>
                                <input
                                    name="vertical"
                                    value={form.vertical}
                                    onChange={onChange}
                                    placeholder="Ex.: Retalho"
                                    required
                                />
                            </div>

                            <div className="form-group">
                                <label>URL Foto Comercial (opcional)</label>
                                <input
                                    name="urlFotoComercial"
                                    value={form.urlFotoComercial}
                                    onChange={onChange}
                                    placeholder="https://..."
                                />
                            </div>
                        </div>

                        <div className="form-group">
                            <label>Nome da Imagem Docker *</label>
                            <input
                                name="nomeImagemDocker"
                                value={form.nomeImagemDocker}
                                onChange={onChange}
                                placeholder="Ex.: myorg/minha-demo:latest"
                                required
                            />
                        </div>

                        <div className="card-footer-actions">
                            <button
                                type="button"
                                className="btn-cancel"
                                onClick={() => navigate("/demos")}
                            >
                                Cancelar
                            </button>
                            <button type="submit" className="btn-primary">
                                Criar demo
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    );
}
